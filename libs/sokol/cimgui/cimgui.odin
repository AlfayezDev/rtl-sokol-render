...
setup :: proc() -> ^Context {
	...
	ctx.def_img = sg.make_image(
		sg.Image_Desc {
			...
			height = 8,
			pixel_format = .RGBA8,
			data = {mip_levels = {0 = {ptr = &def_pixels, size = size_of(def_pixels)}}},
			label = "sokol-imgui-default-image",
		},
	)

	// create texture view for the default image
	ctx.def_view = sg.make_view(
		{texture = {image = ctx.def_img}, label = "sokol-imgui-default-view"},
	)
...
create_fonts_texture :: proc(io: ^imgui.IO) {
	...
	ctx.font_img = sg.make_image(
		sg.Image_Desc {
			...
			height = height,
			pixel_format = .RGBA8,
			data = {mip_levels = {0 = {ptr = pixels, size = uint(width * height) * size_of(u32)}}},
			label = "sokol-imgui-font-image",
		},
	)

	// create texture view for the font image
	ctx.font_view = sg.make_view(
		{texture = {image = ctx.font_img}, label = "sokol-imgui-font-view"},
	)

	io.Fonts.TexID = imtextureid_with_sampler(ctx.font_view, ctx.font_smp)
}

destroy_fonts_texture :: proc() {
	// NOTE: it's valid to call the destroy funcs with sg.INVALID_ID
	sg.destroy_view(ctx.font_view)
	sg.destroy_sampler(ctx.font_smp)
	sg.destroy_image(ctx.font_img)
	ctx.font_view.id = sg.INVALID_ID
...
// Updated to use View instead of Image
imtextureid_with_sampler :: proc(view: sg.View, smp: sg.Sampler) -> imgui.TextureID {
	return (u64(smp.id) << 32) | u64(view.id)
}

imtextureid :: proc(view: sg.View) -> imgui.TextureID {
	return imtextureid_with_sampler(view, ctx.def_smp)
}

view_from_imtextureid :: proc(imtex_id: imgui.TextureID) -> sg.View {
	return sg.View{u32(imtex_id)}
}
...
// Helper to get the parent image from a texture ID (via the view)
image_from_imtextureid :: proc(imtex_id: imgui.TextureID) -> sg.Image {
	view := view_from_imtextureid(imtex_id)
	return sg.query_view_image(view)
}
...
// Updated to use views instead of images
bind_view_sampler :: proc(bindings: ^sg.Bindings, imtex_id: imgui.TextureID) -> sg.Pipeline {
	view := view_from_imtextureid(imtex_id)
	bindings.views[VIEW_tex] = view
	bindings.samplers[SMP_smp] = sampler_from_imtextureid(imtex_id)

	// Get the parent image from the view to check pixel format filterability
	img := sg.query_view_image(view)
	if sg.query_pixelformat(sg.query_image_pixelformat(img)).filter {
		return ctx.def_pip
	} else {
		...
		for &pcmd in commands {
			if pcmd.UserCallback != nil {
				...
				if tex_id != pcmd.TextureId || vtx_offset != pcmd.VtxOffset {
					...
					pip := bind_view_sampler(&bind, tex_id)
